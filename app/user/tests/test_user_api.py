...
class PublicUserApiTests(TestCase):
    ...
    def test_create_user_success(self):
        ...
        user = get_user_model().objects.get(email=test_payload["email"])
        self.assertTrue(user.check_password(test_payload["password"]))
    ...
    def test_password_too_short(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        user_exists = (
            get_user_model().objects.filter(email=test_payload["email"]).exists()
        )
